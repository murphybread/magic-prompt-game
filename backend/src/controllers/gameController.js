...
export const calculateManaCost = (req, res) => {
...
export const castSpell = async (req, res) => {
  ...
  try {
    ...
    const userResult = await pool.query(
      'SELECT mana, level, experience, max_mana FROM users WHERE id = $1',
      [userId]
    );
...
export const getUserMana = async (req, res) => {
  const userId = req.user.id;

  try {
    const result = await pool.query(
      'SELECT mana, max_mana, level, experience, username FROM users WHERE id = $1',
      [userId]
    );
...
export const regenerateMana = async (req, res) => {
  const userId = req.user.id;

  try {
    const result = await pool.query(
      'SELECT mana, max_mana FROM users WHERE id = $1',
      [userId]
    );
